            ...
            for (const toggler of toggelers) {
                try {
                    ...
                    const logos = await Promise.all(parsedData.objects.map(async (obj) => {
                        let width = (obj.width * 0.087 * obj.scaleX).toFixed(2); // Two decimal places
                        let height = (obj.height * 0.087 * obj.scaleY).toFixed(2); // Two decimal places
                        let size = `${width} * ${height}`;
                        
                        if (obj.type === "image") {
                            return { url: obj.src, size };
                            ...
                            try {
                                ...
                                return { url: textDataURL, size };
                            } catch (error) {
                                console.error("Error converting text to data URL:", error);
                                return null; // Skip faulty text object
                            }
                        }
                    }));
